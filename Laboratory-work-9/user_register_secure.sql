...
BEGIN
    ...
    IF LENGTH(password_input) < 19 THEN
        RAISE EXCEPTION 'Password must be at least 19 characters long.';
    END IF;

    -- Перевірка наявності мінімум 2 цифр
    IF LENGTH(REGEXP_REPLACE(password_input, '[^0-9]', '', 'g')) < 2 THEN
